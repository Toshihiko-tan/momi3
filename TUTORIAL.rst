...
For the sake of this tutorial, we will use a simple model with two subpopulations.  
...
Use ``Demography()`` to create a demographic model, ``add_population()`` to add the populations, and ``set_symmetric_migration_rate()`` to set symmetric migration rates.
...
We can then create an instance of the ``Momi3`` class using the simulated tree sequence.  
